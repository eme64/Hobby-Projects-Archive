...
.. must properly save objects when super_chunk unloaded.
.. maybe must see that player loading works smooth at first load ?
.. make sure that all id-counter saved too!
...
Type VOXEL_UNIVERSE
	...
	Function Generate_and_Save(name:String, netw_man:NETWORK_MANAGER, overwrite:Int = False)
		...
		For Local i:Int = 1 To 1
			Local new_world:VOXEL_SUPER_GRID = VOXEL_SUPER_GRID.Create(u, i, 8, VOXEL_GRID.chunk_size_blocks, 3,3, overwrite)
		...
		Select identifyer[0]
			...
			Case 2
				...
				If w Then
					...
					Local c:T_CHEST = T_CHEST(w.object_id_deref.ValueForKey(key))
			...
			Case NETWORK_MANAGER.SERVER
				...
				For Local w:VOXEL_SUPER_GRID = EachIn MapValues(worlds_map)
					...
					If w.grid.last_liquid_refill + VOXEL_GRID.liquid_render_delay < MilliSecs() Then
						...
						For Local o:TPLAYER = EachIn MapValues(w.player_map)
							'Print("player " + o.id)
...
Type VOXEL_SUPER_GRID
	...
	Field object_id_deref:TMap
	Field player_map:TMap
	'Field object_render_map:TMap
	Field block_object_map:TMap ' by global block coordinates
	...
	Method render_objects(netw_man:NETWORK_MANAGER)
		...
		For Local xx:Int = 0 To size_x-1
			For Local yy:Int = 0 To size_y-1
				Local sc:VOXEL_SUPER_CHUNK = super_chunks[xx,yy]
				If sc Then
					
					For Local o:TOBJECT = EachIn MapValues(sc.object_render_map)
						o.render(netw_man)
	...
	Method action_set_full_block_truncated(network_only_if_host:Int , x:Int, y:Int, block_0:Int, block_1:Int, liquid:Int, liquid_plus:Int, liquid_type:Int) ' x,y abs blocks
		...
		sc.action_set_full_block(network_only_if_host, x Mod super_chunk_size_blocks, y Mod super_chunk_size_blocks, block_0, block_1, liquid, liquid_plus, liquid_type)
	End Method
	...
	Function Create:VOXEL_SUPER_GRID(universe:VOXEL_UNIVERSE, world_name:String, super_chunk_size_chunks:Int, chunk_size_blocks:Int, size_x:Int, size_y:Int, overwrite:Int = False)
		...
		sg.object_id_deref = CreateMap()
		sg.player_map = CreateMap()
...
Type VOXEL_SUPER_CHUNK
	...
	Function make_empty:VOXEL_SUPER_CHUNK(super_grid:VOXEL_SUPER_GRID)
		...
		sc.object_render_map = CreateMap()
...
Type BLOCK_OBJECT_MANAGER
	Const TYPE_CHEST:Int = 1		' pattern 1 (3x2)
	Const TYPE_WORKBENCH:Int = 2	' pattern 2 (2x2)
	Const TYPE_TREE:Int = 3			' pattern 3 (1x3)
...
Type TOBJECT
	...
	Field render_sc_x:Int ' strictly local
	Field render_sc_y:Int
	
	Method register()
		super_grid.object_id_deref.Insert(OBJECT_KEY.Create(id), Self)
		
		Local sc_size:Int = super_grid.super_chunk_size_blocks*VOXEL_GRID.block_size
		
		render_sc_x = (Self.x/sc_size)
		render_sc_y = (Self.y/sc_size)
		
		Local sc:VOXEL_SUPER_CHUNK = super_grid.get_super_chunk(render_sc_x, render_sc_y)
		
		sc.object_render_map.Insert(OBJECT_KEY.Create(id), Self)
	End Method
	
	Method de_register()
		super_grid.object_id_deref.Remove(OBJECT_KEY.Create(id))
		
		Local sc:VOXEL_SUPER_CHUNK = super_grid.get_super_chunk(render_sc_x, render_sc_y)
		
		sc.object_render_map.Remove(OBJECT_KEY.Create(id))
	End Method
	
	Method re_register()
		Local sc_size:Int = super_grid.super_chunk_size_blocks*VOXEL_GRID.block_size
		
		Local new_render_sc_x:Int = (Self.x/sc_size)
		Local new_render_sc_y:Int = (Self.y/sc_size)
		
		new_render_sc_x = Max(0, new_render_sc_x)
		new_render_sc_y = Max(0, new_render_sc_y)
		
		new_render_sc_x = Min(new_render_sc_x, super_grid.size_x-1)
		new_render_sc_y = Min(new_render_sc_y, super_grid.size_y-1)
		
		If new_render_sc_x <> render_sc_x Or new_render_sc_y <> render_sc_y Then
			Local old_sc:VOXEL_SUPER_CHUNK = super_grid.get_super_chunk(render_sc_x, render_sc_y)
			old_sc.object_render_map.Remove(OBJECT_KEY.Create(id))
			
			render_sc_x = new_render_sc_x
			render_sc_y = new_render_sc_y
	...
	Method render_collision()
			...
			If Ceil((Self.y + Self.ry)/VOXEL_GRID.block_size) < Ceil((Self.y+Self.ry + Self.vy)/VOXEL_GRID.block_size) Then
				...
				'muss +vx haben, damit keine störunen wenn genau auf ecke des blockes fällt
	...
	Method get_super_chunks(diameter:Int, x1:Int Var, x2:Int Var, y1:Int Var, y2:Int Var)
		...
		x2 = Min(x2, super_grid.size_x-1)
		y2 = Min(y2, super_grid.size_y-1)
	End Method
	...
	Function stream_to_ignore(stream:TStream, netw_man:NETWORK_MANAGER)
		stream_to_super_grid(stream, netw_man, Null)
	End Function
	
	Function stream_to_super_grid(stream:TStream, netw_man:NETWORK_MANAGER, super_grid:VOXEL_SUPER_GRID)
		'this here is no good. must search in Global directory ! -> If Not found add locally here.
		...
		Repeat
			...
			Local o:TOBJECT' = TOBJECT(super_grid.object_id_deref.ValueForKey(key))
			
			If super_grid Then o = TOBJECT(super_grid.object_id_deref.ValueForKey(key))
			...
			Select netw_man.mode
				Case NETWORK_MANAGER.CLIENT
					...
					Else
						If rcv_owner = netw_man.my_id Then
							' my object
							o = TOBJECT.create_from_stream(stream, rcv_id, rcv_owner, rcv_typ, super_grid)
							If super_grid And o Then o.register()'map.Insert(key, o)
						Else
							' other's object
							o = TOBJECT.create_from_stream(stream, rcv_id, rcv_owner, rcv_typ, super_grid)
							If super_grid And o Then o.register()'map.Insert(key, o)
	...
	Function create_from_stream:TOBJECT(stream:TStream, id:Int, owner:Int, typ:Int, super_grid:VOXEL_SUPER_GRID)
		Select typ
...
Type TITEM_DROP Extends TOBJECT
	...
	Method render(netw_man:NETWORK_MANAGER)
		...
		For Local player:TPLAYER = EachIn MapValues(super_grid.player_map) ' maybe have extra player list for efficiency ?
		...
		If netw_man.mode = NETWORK_MANAGER.SERVER And ITEM.countable[item_kind] And (Not to_be_removed) Then
			...
			For Local drop:TITEM_DROP = EachIn MapValues(super_grid.player_map) ' maybe have extra player list for efficiency ?
...
Type TPLAYER Extends TOBJECT
	...
	Method register()
		...
		super_grid.player_map.Insert(OBJECT_KEY.Create(id), Self)
	End Method
	
	Method de_register()
		Super.de_register()
		
		super_grid.player_map.Remove(OBJECT_KEY.Create(id))
	End Method
...
Type TBLOCK_OBJECT Extends TOBJECT
	Field block_x:Int
	Field block_y:Int
	...
	Method register()
		...
		super_grid.block_object_map.Insert(COORD_KEY.Create(block_x, block_y), Self)
	End Method
...
Type T_CHEST Extends TBLOCK_OBJECT
	Global img:TImage
...
Type T_S_Client
	...
	Function Create:T_S_Client(ip:Int, port:Int, world:VOXEL_SUPER_GRID)
		...
		kind = ITEM.BO_WORKBENCH
		amount = 5
		c.inventory.add_item(kind,amount,info)
		
		kind = ITEM.BO_SAPLING
		amount = 50
		c.inventory.add_item(kind,amount,info)
	...
	Method update(universe:VOXEL_UNIVERSE)
		...
		If clientside_port And (MilliSecs() - last_update_objects_to > 100) Then
			...
			If player Then
				...
				For Local xx:Int = x1 To x2
					For Local yy:Int = y1 To y2
						...
						sc.time_since_last_use = MilliSecs()
						Package_Manager.write_block_update(send_stream, sc.block_update)
						Package_Manager.write_objects(send_stream, world.id, sc.object_render_map, xx, yy, universe.netw_man)
					Next
				Next
			EndIf
			...
			Package_Manager.finish_packets(send_stream)
...
Type ITEM ' no data, only functions !
	...
	Const CRAFT_LOG:Int = 10
	Const BO_WORKBENCH:Int = 11
	Const BO_SAPLING:Int = 12
	
	Const item_count:Int = 13
	...
	Function init()
		...
		set_item_simple(CRAFT_LOG, 			True,  	"log")
		
		set_item_block_object(BO_WORKBENCH, 	True, 		"bo_workbench", 				BLOCK_OBJECT_MANAGER.TYPE_WORKBENCH)
		set_item_block_object(BO_SAPLING, 		True, 		"bo_sapling", 					BLOCK_OBJECT_MANAGER.TYPE_TREE)
	End Function
...
Type Package_Manager
	Function execute(Stream:TUDPStream, client:T_S_Client, universe:VOXEL_UNIVERSE, netw_man:NETWORK_MANAGER)
		...
		Select code
			...
			Case 3001 ' objects update
				...
				If universe Then
					Local rcv_world_id:Int = Stream.ReadInt()
					Local rcv_sc_x:Int = Stream.ReadInt()
					Local rcv_sc_y:Int = Stream.ReadInt()
					Local super_grid:VOXEL_SUPER_GRID = universe.find_world_if_loaded(rcv_world_id)
					
					If super_grid Then
						If super_grid.super_chunk_exists(rcv_sc_x, rcv_sc_y) Then
							'Local sc:VOXEL_SUPER_CHUNK = super_grid.get_super_chunk(rcv_sc_x, rcv_sc_y)
							TOBJECT.stream_to_super_grid(Stream, netw_man, super_grid) ' this is so flawed !
							Package_Manager.read_safety(Stream)
						Else
							Print("flush 3")
							Local m:TMap = CreateMap()
							TOBJECT.stream_to_ignore(Stream, netw_man)
							Package_Manager.read_safety(Stream)
						EndIf
					Else
						Select netw_man.mode
	...
	Function write_objects(Stream:TUDPStream, world_id:Int, map:TMap, sc_x:Int, sc_y:Int, netw_man:NETWORK_MANAGER)
	...
	Function finish_packets(Stream:TUDPStream)
		Stream.SendMsg()
	End Function
		...
		Select mode
			...
			Case NETWORK_MANAGER.CLIENT
				If client_last_send + 100 < MilliSecs() Then
					...
					If client_connector.player Then
						Local x1:Int, x2:Int, y1:Int, y2:Int
						
						client_connector.player.get_super_chunks(2, x1, x2, y1, y2)
						For Local xx:Int = x1 To x2
							For Local yy:Int = y1 To y2
								Local sc:VOXEL_SUPER_CHUNK = client_connector.world.get_super_chunk(xx,yy, False)
								Package_Manager.write_objects(client_connector.send_stream, client_connector.world.id, sc.object_render_map, xx, yy, Self)
							Next
						Next
					EndIf
					
					'Package_Manager.write_objects(client_connector.send_stream, client_connector.world.id, client_connector.world.object_map, Self)
				EndIf
				
				Package_Manager.finish_packets(client_connector.send_stream)
