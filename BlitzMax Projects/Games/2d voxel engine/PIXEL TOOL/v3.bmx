...
Const GENERATION_MODE:Int = 1 ' 0 = scale, 1 = borders, 2 = shadow
...
Global COLOR_SHADOW:Int [] = [..
get_pixel_bytes_reverse([Byte(255),Byte(0),Byte(0),Byte(255)]),..
get_pixel_bytes_reverse([Byte(220),Byte(220),Byte(220),Byte(255)]),..
get_pixel_bytes_reverse([Byte(190),Byte(190),Byte(190),Byte(255)]),..
get_pixel_bytes_reverse([Byte(150),Byte(150),Byte(150),Byte(255)]),..
get_pixel_bytes_reverse([Byte(150),Byte(150),Byte(150),Byte(255)])]
...
Function shadow_get_empty:TPixmap[](size:Int, n:Int) ' get empty square canvas
	Local dest:TPixmap[] = New TPixmap[n]
	...
	For Local i:Int = 0 To n-1
		...
		For Local x_out:Int = 0 To size-1
			For Local y_out:Int = 0 To size-1
				dest[i].WritePixel(x_out, y_out,  0) ' 0 = empty
			Next
		Next
	Next
...
Function shadow_write_top(src:TPixmap[])
	For Local i:Int = 0 To src.length-1
		For Local x_out:Int = 0 To src[i].width-1
			src[i].WritePixel(x_out, 0,  src[i].width)
		Next
	Next
End Function

Function shadow_write_bottom(src:TPixmap[])
	For Local i:Int = 0 To src.length-1
		For Local x_out:Int = 0 To src[i].width-1
			src[i].WritePixel(x_out, src[i].width-1,  src[i].width)
		Next
	Next
End Function

Function shadow_write_left(src:TPixmap[])
	For Local i:Int = 0 To src.length-1
		For Local y_out:Int = 0 To src[i].width-1
			src[i].WritePixel(0, y_out,  src[i].width)
		Next
	Next
End Function

Function shadow_write_right(src:TPixmap[])
	For Local i:Int = 0 To src.length-1
		For Local y_out:Int = 0 To src[i].width-1
			src[i].WritePixel(src[i].width-1, y_out, src[i].width)
		Next
	Next
End Function

Function shadow_write_corner_tl(src:TPixmap[])
	For Local i:Int = 0 To src.length-1
		src[i].WritePixel(0, 0, src[i].width)
	Next
End Function

Function shadow_write_corner_tr(src:TPixmap[])
	For Local i:Int = 0 To src.length-1
		src[i].WritePixel(src[i].width-1, 0, src[i].width)
	Next
End Function

Function shadow_write_corner_bl(src:TPixmap[])
	For Local i:Int = 0 To src.length-1
		src[i].WritePixel(0, src[i].width-1, src[i].width)
	Next
End Function

Function shadow_write_corner_br(src:TPixmap[])
	For Local i:Int = 0 To src.length-1
		src[i].WritePixel(src[i].width-1, src[i].width-1, src[i].width)
	Next
End Function

Function get_pixmap_from_shadow:TPixmap[](canv:TPixmap[])
	Local dest:TPixmap[] = New TPixmap[canv.length]
	
	For Local i:Int = 0 To canv.length-1
		dest[i] = CreatePixmap(canv[0].width, canv[0].height, PF_BGRA8888)
		
		' copy
		For Local x_out:Int = 0 To canv[0].width-1
			For Local y_out:Int = 0 To canv[0].height-1
				Local clr_canv:Int = canv[i].ReadPixel(x_out, y_out)
				
				dest[i].WritePixel(x_out, y_out,  canv[i].ReadPixel(x_out, y_out))
			Next
		Next
		
		' calculate
		Local did_work:Int = True
		...
		While did_work
			...
			For Local x_out:Int = 0 To canv[0].width-1
				For Local y_out:Int = 0 To canv[0].height-1
					Local clr_old:Int = dest[i].ReadPixel(x_out, y_out)
					
					Local clr_new:Int = clr_old
					
					For Local xx:Int = Max(x_out-1, 0) To Min(canv[0].width-1, x_out+1)
						For Local yy:Int = Max(y_out-1, 0) To Min(canv[0].width-1, y_out+1)
							clr_new = Max(clr_new, dest[i].ReadPixel(xx,yy)-1)
						Next
					Next
					
					If clr_new <> clr_old Then did_work = True
					
					dest[i].WritePixel(x_out, y_out,  clr_new)
		...
		For Local x_out:Int = 0 To canv[0].width-1
			For Local y_out:Int = 0 To canv[0].height-1
				Local clr_canv:Int = dest[i].ReadPixel(x_out, y_out)
				
				Local clr_set:Int = COLOR_SHADOW[clr_canv]
				
				dest[i].WritePixel(x_out, y_out,  clr_set)
		...
		Else
			...
			Select GENERATION_MODE
				...
				Case 2 ' shadow
					...
					Rem
					shadow_write_right(src:TPixmap[])
	For Local i:Int = 0 To src.length-1
		For Local y_out:Int = 0 To src[i].width-1
			src[i].WritePixel(src[i].width-1, y_out, src[i].width)
		Next
...
Function get_pixmap_from_shadow:TPixmap[](canv:TPixmap[])
					...
					'------------------------------------------------------------------------- TOP LEFT
					Local canvas:TPixmap[] = shadow_get_empty(resource.height/2, res_frames.length)
					
					shadow_write_corner_tl(canvas)
					Local destination_tl_0:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_tl_0)), dest_path + "/tl_0.png",5)
					
					shadow_write_top(canvas)
					Local destination_tl_1:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_tl_1)), dest_path + "/tl_1.png",5)
					
					shadow_write_left(canvas)
					Local destination_tl_3:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_tl_3)), dest_path + "/tl_3.png",5)
					
					canvas = shadow_get_empty(resource.height/2, res_frames.length)
					shadow_write_left(canvas)
					Local destination_tl_2:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_tl_2)), dest_path + "/tl_2.png",5)
					
					'------------------------------------------------------------------------- TOP Right
					canvas = shadow_get_empty(resource.height/2, res_frames.length)
					
					shadow_write_corner_tr(canvas)
					Local destination_tr_0:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_tr_0)), dest_path + "/tr_0.png",5)
					
					shadow_write_top(canvas)
					Local destination_tr_1:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_tr_1)), dest_path + "/tr_1.png",5)
					
					shadow_write_right(canvas)
					Local destination_tr_3:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_tr_3)), dest_path + "/tr_3.png",5)
					
					canvas = shadow_get_empty(resource.height/2, res_frames.length)
					shadow_write_right(canvas)
					Local destination_tr_2:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_tr_2)), dest_path + "/tr_2.png",5)
					
					'------------------------------------------------------------------------- BOTTOM LEFT
					canvas = shadow_get_empty(resource.height/2, res_frames.length)
					
					shadow_write_corner_bl(canvas)
					Local destination_bl_0:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_bl_0)), dest_path + "/bl_0.png",5)
					
					shadow_write_left(canvas)
					Local destination_bl_1:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_bl_1)), dest_path + "/bl_1.png",5)
					
					shadow_write_bottom(canvas)
					Local destination_bl_3:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_bl_3)), dest_path + "/bl_3.png",5)
					
					canvas = shadow_get_empty(resource.height/2, res_frames.length)
					shadow_write_bottom(canvas)
					Local destination_bl_2:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_bl_2)), dest_path + "/bl_2.png",5)
					
					'------------------------------------------------------------------------- BOTTOM Right
					canvas = shadow_get_empty(resource.height/2, res_frames.length)
					
					shadow_write_corner_br(canvas)
					Local destination_br_0:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_br_0)), dest_path + "/br_0.png",5)
					
					shadow_write_right(canvas)
					Local destination_br_1:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_br_1)), dest_path + "/br_1.png",5)
					
					shadow_write_bottom(canvas)
					Local destination_br_3:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_br_3)), dest_path + "/br_3.png",5)
					
					canvas = shadow_get_empty(resource.height/2, res_frames.length)
					shadow_write_bottom(canvas)
					Local destination_br_2:TPixmap[] = get_pixmap_from_shadow(canvas)
					SavePixmapPNG(scale_pixmap(pixmap_merge_frames(destination_br_2)), dest_path + "/br_2.png",5)
