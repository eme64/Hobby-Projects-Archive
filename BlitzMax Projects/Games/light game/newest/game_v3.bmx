...
Type TLevel
	...
	Function Create:TLevel()
		...
		Local data:Int[,] = TLevel.generate_data(l.table_x, l.table_y, l)'New Int[l.table_x, l.table_y]
		
		Local blocks:TBlock[] = New TBlock[4]
		
		blocks[0] = TBlock.Create(1, 1, 1)
		blocks[1] = TBlock.Create(0, 0, 2)
		blocks[2] = TBlock.Create(0, 0, 3)
		blocks[3] = TBlock.Create(0, 0, 4)
		
		For Local x:Int = 0 To l.table_x-1
			For Local y:Int = 0 To l.table_y-1
				If x = 0 Or x = l.table_x-1 Or y = 0 Or y = l.table_y-1 Then
					l.table[x,y] = blocks[0]
				Else
					l.table[x,y] = blocks[data[x,y]]
	...
	Function generate_data:Int[,](dx:Int,dy:Int, l:TLevel)
...
Type TBlock
	...
	Function Create:TBlock(collision:Int=0, fg:Int=0, bg:Int=0)
...
Type TLamp Extends TObject
	...
	Method draw()
		SetColor 255,255,255
		Draw_Image TLamp.image, GAME.level.ansicht_act_x + Self.x, GAME.level.ansicht_act_y + Self.y, (MilliSecs() / 500) Mod 4
	End Method
...
Type TPlayer Extends TMob
	...
	Function Create:TPlayer(x:Float,y:Float,l:TLevel)
		...
		p.light = TLight.Create(TLight.source_spot,0,0)
	...
	Method render()
		...
		Self.vx:+ KeyDown(key_d) - KeyDown(key_a)
		Self.vy:+ KeyDown(key_s) - KeyDown(key_w)
