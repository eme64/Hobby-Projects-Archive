...
Type TERRAIN
	...
	Global wasserstand:Int
	Global wasser_image:TImage
	
	Function ini()
		TERRAIN.wasser_image=LoadImage("gfx\hintergrund\wasser.png")
	End Function
	...
	Function Create(x:Int,y:Int,art:Int=0,art2:Int=0,wasserstand:Int=580)
		...
		TERRAIN.wasserstand=wasserstand
...
Type PLAYER
	...
	Method render()
		...
		Self.v_x:+Self.a_x'kräfte addieren zur geschwindigkeit
		Self.v_y:+Self.a_y
		Self.a_x=0
		Self.a_y=0
		...
		If Self.v_x+Self.versch_x>0 Then
			For Local yy:Int=-10 To 10
				For Local xx:Int=0 To Self.v_x+Self.versch_x
					If PIXELS.LEER<>TERRAIN.read(Self.x+5+xx,Self.y+yy) Then
						Self.collided[2]=1
						Exit
					End If
				Next
				If Self.collided[2]=1 Then Exit
			Next
		...
		If Self.v_x+Self.versch_x<0 Then
			For Local yy:Int=-10 To 10
				For Local xx:Int=0 To Self.v_x+Self.versch_x Step -1
					If PIXELS.LEER<>TERRAIN.read(Self.x-5+xx,Self.y+yy) Then
						Self.collided[3]=1
						Exit
					End If
				Next
				If Self.collided[3]=1 Then Exit
			Next
		...
		If Self.v_y+Self.versch_y>0 Then
			For Local xx:Int=-5 To 5
				For Local yy:Int=0 To Self.v_y+Self.versch_y
					If PIXELS.LEER<>TERRAIN.read(Self.x+xx,Self.y+10+yy) Then
						Self.collided[0]=1
						Exit
					End If
				Next
				If Self.collided[0]=1 Then Exit
			Next
		...
		If Self.v_y+Self.versch_y<0 Then
			For Local xx:Int=-5 To 5
				For Local yy:Int=0 To Self.v_y+Self.versch_y Step -1
					If PIXELS.LEER<>TERRAIN.read(Self.x+xx,Self.y-10+Self.v_y+Self.versch_y) Then
						Self.collided[1]=1
						Exit
					End If
				Next
				If Self.collided[1]=1 Then Exit
			Next
	...
	Method draw(x:Int,y:Int)'parameter für verschiebungen
	...
	Method draw_screen(x:Int,y:Int,b:Int,h:Int)
		...
		DrawImage TERRAIN.wasser_image,x,y+TERRAIN.wasserstand-Self.y+h/2
		...
		DrawText Self.steuerung.selected_waffe,x+200,y
	...
	Method render()
		...
		Else
			If Self.last_usebutton=1 Then
				...
				Select Self.selected_waffe
					Case 0
						GRANATE_NORMAL.Create(Self.player.x,Self.player.y,Self.player.winkel,Self.player.power)
					Case 1
						GRANATE_FLASH.Create(Self.player.x,Self.player.y,Self.player.winkel,Self.player.power)
					Case 2
						GRANATE_FEUER.Create(Self.player.x,Self.player.y,Self.player.winkel,Self.player.power)
					Case 3
						RAKETE_NORMAL.Create(Self.player.x,Self.player.y,Self.player.winkel,Self.player.power)
					Default
						Self.selected_waffe=0
				End Select
			End If
		...
		If KeyHit(Self.keys[4]) Then
			Self.selected_waffe:+1
			If Self.selected_waffe>3 Then Self.selected_waffe=0
		End If
	End Method
...
Type GRANATE_NORMAL Extends OBJEKT
	...
	Method render()
		...
		'rechtskollision
		If Self.v_x>0 Then
			For Local x:Int=0 To Self.v_x+2
				If PIXELS.LEER<>TERRAIN.read(Self.x+x,Self.y) Then
					Self.v_x:*-0.3
					Exit
				End If
			Next
		...
		'linkskollision
		If Self.v_x<0 Then
			For Local x:Int=0 To Self.v_x-2 Step -1
				If PIXELS.LEER<>TERRAIN.read(Self.x+x,Self.y) Then
					Self.v_x:*-0.3
					Exit
				End If
		...
		'bodenkollision
		If Self.v_y>0 Then
			For Local y:Int=0 To Self.v_y+2
				If PIXELS.LEER<>TERRAIN.read(Self.x,Self.y+y) Then
					Self.v_y:*-0.3
					Exit
				End If
			Next
		...
		'deckenkollision
		If Self.v_y<0 Then
			For Local y:Int=0 To Self.v_y-2 Step -1
				If PIXELS.LEER<>TERRAIN.read(Self.x,Self.y+y) Then
					Self.v_y:*-0.3
					Exit
				End If
		...
		If Self.timer<=0 Then
			TERRAIN.loch(Self.x,Self.y,25,28,PIXELS.rgba(10,10,10,255))
			ListRemove(OBJEKT.list,Self)
			EXPOSIONS.Create(Self.x,Self.y,0,0,EXPOSIONS.explosion_1,25,1,1,20,255,255,100,255)
			PLAYER.render_explosion_all(Self.x,Self.y,150,25,10)
		End If
		
		If Self.y>TERRAIN.wasserstand Then
			ListRemove(OBJEKT.list,Self)'pflatsch
			SPRITZ.Create(Self.x,TERRAIN.wasserstand,SPRITZ.image_1,1,1,50,100,200,255,0.8)
		End If
...
Type GRANATE_FEUER Extends OBJEKT
	Field timer:Int=120
	
	Global image:TImage
	
	Function ini()
		GRANATE_FEUER.image=LoadImage("gfx\objekte\granate_feuer.png")
		MidHandleImage GRANATE_FEUER.image
	End Function
	
	Function Create:GRANATE_FEUER(x:Float,y:Float,w:Float,power:Float,timer:Int=60)
		Local g:GRANATE_FEUER=New GRANATE_FEUER
		g.x=x
		g.y=y
		g.v_x=Cos(w)*power*0.2
		g.v_y=Sin(w)*power*0.2
	...
	Method render()
		...
		If Self.timer<=0 Then
			...
			PLAYER.render_explosion_all(Self.x,Self.y,50,5,5)
			
			For Local i:Int=1 To Rand(30,40)
	...
	Method draw(x:Int,y:Int)'parameter für verschiebungen
		SetColor 255,255,255
		'SetRotation ATan2(Self.v_y,Self.v_x)'nicht bei granate
		DrawImage GRANATE_FEUER.image,Self.x+x,Self.y+y
		'SetRotation 0'nicht bei granate
	End Method
End Type
...
Type GRANATE_FLASH Extends OBJEKT
	Field timer:Int=120
	
	Global image:TImage
	
	Function ini()
		GRANATE_FLASH.image=LoadImage("gfx\objekte\granate_flash.png")
		MidHandleImage GRANATE_FLASH.image
	End Function
	
	Function Create:GRANATE_FLASH(x:Float,y:Float,w:Float,power:Float,timer:Int=60)
		Local g:GRANATE_FLASH=New GRANATE_FLASH
		g.x=x
		g.y=y
		g.v_x=Cos(w)*power*0.2
		g.v_y=Sin(w)*power*0.2
	...
	Method render()
		...
		If Self.timer<=0 Then
			...
			PLAYER.render_explosion_all(Self.x,Self.y,100,1,50)'flash=20,schaden=5
	...
	Method draw(x:Int,y:Int)'parameter für verschiebungen
		SetColor 255,255,255
		'SetRotation ATan2(Self.v_y,Self.v_x)'nicht bei granate
		DrawImage GRANATE_FLASH.image,Self.x+x,Self.y+y
		'SetRotation 0'nicht bei granate
	End Method
End Type
...
Type RAKETE_NORMAL Extends OBJEKT
	...
	Method render()
		...
		Local collided:Int=0
		
		If Self.v_x>1 Then
			For Local x:Int=0 To Self.v_x
				If PIXELS.LEER<>TERRAIN.read(Self.x+x,Self.y+Float(x*Self.v_y/Self.v_x)) Then
					collided=1
					Self.x:+x
					Self.y:+Float(x*Self.v_y/Self.v_x)
					Exit
				End If
			Next
		Else
			For Local x:Int=0 To Self.v_x Step -1
				If PIXELS.LEER<>TERRAIN.read(Self.x+x,Self.y+Float(x*Self.v_y/Self.v_x)) Then
					collided=1
					Self.x:+x
					Self.y:+Float(x*Self.v_y/Self.v_x)
...
Type FEUER Extends OBJEKT
	...
	Method render()
		...
		For Local x:Int=-4+Self.v_x To 4+Self.v_x
			For Local y:Int=0+Self.v_y To 4+Self.v_y
				If PIXELS.LEER<>TERRAIN.read(Self.x+x,Self.y+y) Then
					collided=1
					Exit
				End If
			Next
			If collided=1 Then Exit
		Next
		...
		If TERRAIN.wasserstand<Self.y Then
			ListRemove(OBJEKT.list,Self)'zischlaut
			SPRITZ.Create(Self.x,TERRAIN.wasserstand,SPRITZ.image_1,Rnd()*0.5+0.5,Rnd()*0.5+0.5,50,100,200,255,0.5)
		End If
	End Method
...
Type SPRITZ Extends SPECIALS
	
	Global image_1:TImage
	
	Field alpha_2:Float
	
	Function ini()
		SPRITZ.image_1=LoadImage("gfx\sonstiges\wasser_spritz.png")
		MidHandleImage SPRITZ.image_1
	End Function
	
	Function Create:SPRITZ(x:Float,y:Float,image:TImage,x_scale:Float,y_scale:Float,count:Int,r:Byte,g:Byte,b:Byte,a:Float)
		Local s:SPRITZ = New SPRITZ
	...
	Method render()
		...
		Self.alpha=Self.alpha_2*Float(Self.count-Self.counter)/Float(Self.count)
